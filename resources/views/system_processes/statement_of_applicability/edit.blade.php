            ...
            @foreach($groups as $group)
                ...
                @foreach($fields as $field)
                    @if($group->id == $field->soaField->soa_field_group_id)
                        <div id="{{ $field->id }}" class="flex flex-wrap border-b-2 py-2 my-2" :class="{'': open{{ $group->id }}, 'hidden': ! open{{ $group->id }} }">
                            ...
                            <div class="w-full sm:w-1/5">
                                ...
                                <textarea class="text-xs sm:text-sm appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="comment" name="{{ $field->id }}[comment]" required oninvalid="this.setCustomValidity('{{__("Popunite polje")}}')" oninput="this.setCustomValidity('')">{{ $field->comment }}</textarea>
                                @error('comment')
                                    <span class="text-red-700 italic text-sm">{{ $message }}</span>
                                @enderror
                            </div>
