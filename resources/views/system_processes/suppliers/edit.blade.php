<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl mb-0 text-gray-800 leading-tight">
            {{ session('standard_name') }} - {{ __('Odobreni isporučioci') }} - {{ __('Izmena') }}
        </h2>
    </x-slot>
    ...
    <div class="mx-auto md:w-3/5 mt-1 md:p-10 sm:p-2 rounded">
		...
		<form action="{{ route('suppliers.update', $supplier->id) }}" method="POST" autocomplete="off" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div class="mb-4">
				...
				<input type="text" class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="supplier_name" name="supplier_name" value="{{ $supplier->supplier_name }}" autofocus required oninvalid="this.setCustomValidity('{{ __("Popunite polje") }}')" oninput="this.setCustomValidity('')">
				@error('supplier_name')
			...
			<div class="mb-4">
                ...
                <textarea class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="subject" name="subject" required oninvalid="this.setCustomValidity('{{ __("Popunite polje") }}')" oninput="this.setCustomValidity('')">{{ $supplier->subject }}</textarea>
				@error('subject')
					<span class="text-red-700 italic text-sm">{{ __($message) }}</span>
				@enderror
			</div>
