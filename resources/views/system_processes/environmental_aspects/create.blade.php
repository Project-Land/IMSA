<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ session('standard_name') }} - {{ __('Aspekti životne sredine - Kreiranje') }}
        </h2>
    </x-slot>
    ...
    <div class="mx-auto md:w-3/5 mt-1 md:p-10 sm:p-2 rounded">
		...
		<form action="{{ route('environmental-aspects.store') }}" method="POST" autocomplete="off" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
			...
			<div class="mb-4">
				...
				<input type="text" class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="process" name="process" autofocus required oninvalid="this.setCustomValidity('Popunite polje')" oninput="this.setCustomValidity('')">
				@error('process')
            ...
            <div class="mb-4">
				...
				<input type="text" class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="waste" name="waste" required oninvalid="this.setCustomValidity('Popunite polje')" oninput="this.setCustomValidity('')">
				@error('waste')
					<span class="text-red-700 italic text-sm">{{ $message }}</span>
				@enderror
            </div>
            ...
            <div class="mb-4">
				...
				<input type="text" class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="aspect" name="aspect" required oninvalid="this.setCustomValidity('Popunite polje')" oninput="this.setCustomValidity('')">
				@error('aspect')
					<span class="text-red-700 italic text-sm">{{ $message }}</span>
				@enderror
            </div>
            ...
            <div class="mb-4">
				...
				<input type="text" class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="influence" name="influence" required oninvalid="this.setCustomValidity('Popunite polje')" oninput="this.setCustomValidity('')">
				@error('influence')
					<span class="text-red-700 italic text-sm">{{ $message }}</span>
				@enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select class="block appearance-none w-full border border-gray-200 text-gray-700 py-2 px-3 pr-8 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" name="waste_type" id="waste_type" required>
                    <option value="1">Opasan</option>
                    <option value="0">Neopasan</option>
                </select>
            </div>
