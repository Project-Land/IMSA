<x-app-layout>
    ...
    <form id="document-edit" action="{{ route('certification-documents.update', $document->id) }}" method="POST" enctype="multipart/form-data" class="bg-white mx-auto md:w-3/5 mt-2 md:mt-1 shadow-md rounded px-8 pt-6 pb-4 mb-4">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="name" class="block text-gray-700 text-sm font-bold mb-2">{{__('Naziv sertifikata')}}:</label>
            <input type="text" class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="name" name="name" value="{{ $document->name }}" autofocus required>
            @error('name')
        ...
        <div class="mb-4">
            ...
            <input readonly class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="start_date" id="start_date" type="text" value="{{ date('d.m.Y', strtotime($document->start_date)) }}" autocomplete="off" placeholder="xx.xx.xxxx" required>
            @error('start_date')
        ...
        <div class="mb-4">
            ...
            <input readonly class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="end_date" id="end_date" type="text" value="{{ date('d.m.Y', strtotime($document->end_date)) }}" autocomplete="off" placeholder="xx.xx.xxxx" required>
            @error('end_date')
                <span class="text-red-700 italic text-sm">{{ $message }}</span>
            @enderror
        </div>
