<x-app-layout>
    ...
    <form action="{{ route('sectors.update', $sector->id) }}" method="POST" class="bg-white mx-auto md:w-3/5 mt-2 md:mt-1 shadow-md rounded px-8 pt-6 pb-4 mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="text" class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="name" name="name" value="{{ $sector->name }}" autofocus required oninvalid="this.setCustomValidity('{{ __('Naziv nije popunjen') }}')"
