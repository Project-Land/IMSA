...
class PlanIpController extends Controller
{
    ...
    public function update(UpdatePlanIpRequest $request, $id)
    {
        ...
        try{
            $planIp->save();
            CustomLog::info('Plan IP id-"'.$planIp->id.'" je sačuvan. Korisnik: '.\Auth::user()->name.', '.\Auth::user()->email.', '.date('d.m.Y').' u '.date('H:i:s'), \Auth::user()->currentTeam->name);
            $request->session()->flash('status', 'Dokument je uspešno uklonjen');
        } catch(Exception $e){
            CustomLog::warning('Neuspeli pokušaj izmene plana IP id-'.$planIp->id.'. Korisnik: '.\Auth::user()->name.', '.\Auth::user()->email.', '.date('d.m.Y').' u '.date('H:i:s').' Greška- '.$e->getMessage(), \Auth::user()->currentTeam->name);
    ...
    public function destroy($id)
    {
        ...
        try{
            PlanIp::destroy($id);
            CustomLog::info('Plan IP id-'.$plan_ip->id.' je uklonjen. Korisnik: '.\Auth::user()->name.', '.\Auth::user()->email.', '.date('d.m.Y').' u '.date('H:i:s'), \Auth::user()->currentTeam->name);
            return back()->with('status', 'Plan IP je uspešno uklonjen');
        } catch(Exception $e){
            CustomLog::warning('Neuspeli pokušaj brisanja plana IP id-'.$plan_ip->id.'. Korisnik: '.\Auth::user()->name.', '.\Auth::user()->email.', '.date('d.m.Y').' u '.date('H:i:s').' Greška- '.$e->getMessage(), \Auth::user()->currentTeam->name);
